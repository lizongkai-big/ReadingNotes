...
3. Java “==”与“equals”的区别 [[link](http://www.cnblogs.com/zhxhdean/archive/2011/03/25/1995431.html)]
   ...
   1. **基本数据类型**（ 只有且仅有这八种：byte，short，char，int，long，float，double，boolean，无String ）
   2. **引用数据类型**：类、接口类型、数组类型、枚举类型、注解类型。
   3. “==” 对于基本数据类型比较“值”，对于复合数据类型比较“对象的地址”
   4. “equals” 是一个对象的方法，未被重写（Object中）比较的是“对象的地址”，相当于“==”；重写以后一般比较“类中的变量的值”
   5. String是一种不允许变化的类“==”原本比较的是**内存中的存放地址，** JAVA当中所有的类都是继承于Object这个基类的，在Object中的基类中定义了一个equals的方法，这个方法的初始行为是比较对象的内存地 址，但在一些类库当中这个方法被覆盖掉了，如String,Integer,Date在这些类当中equals有其自身的实现，而不再是比较类在堆内存中的存放地址了。**“equals”比较值**
   6. null == null  TRUE
   7. "=="符号在比较对象的时候是比较的内存地址，而对于原始数据类型是直接比对的数据值。

4. [hashCode() 与 equals() 的区别](https://www.cnblogs.com/skywang12345/p/3324958.html)

   1. 无“类对应的散列表”。没啥关系，因为用不到hashCode()
   2. 有“类对应的散列表”。`如果两个对象的hashcode() 相同，equals()未必为true；equals() 为true，hashCode() 一定相等` 在equals未被重写的时候，该规则是符合的；而equals()被重写之后，两个对象的内存地址不同，但是数值相同，所以equals()为true，但是hashCode()不同，所以不符合规则。因此重写equals的时候也要重写hashCode()，让hashCode()的返回值与对象的数值有关

5. System.out.println(Object) 与 toString() 的关系 【[link](https://stackoverflow.com/questions/29318996/the-connection-between-system-out-println-and-tostring-in-java)】
...
10. String 字符串分割【[link](https://blog.csdn.net/kunlong0909/article/details/8507290)】

11. collection

12. Java 集合--Map、HashMap、HashTable、TreeMap【[link](http://zzqrj.iteye.com/blog/841782)】
...
13. HashMap 工作原理 【[link](https://yikun.github.io/2015/04/01/Java-HashMap%E5%B7%A5%E4%BD%9C%E5%8E%9F%E7%90%86%E5%8F%8A%E5%AE%9E%E7%8E%B0/)】

    1. HashMap在每个节点中储存**键值对对象 **Map.Entry
    2. hashing实现 (h = k.hashCode()) ^ (h >>> 16)
    3. ​

14. Java 8 中的 Streams API 详解 [[link](https://www.ibm.com/developerworks/cn/java/j-lo-java8streamapi/)] --+

15. Java8：不要再用循环了 [link]  <--------+
...
18. Item 泛型，实例化的时候让其为何种类型就是何种类型

19. java native方法及JNI实例 [[link](https://blog.csdn.net/xw13106209/article/details/6989415)]  被native修饰的方法不是由java实现的，是依赖系统底层的

20. transient 【[link](http://www.cnblogs.com/lanxuezaipiao/p/3369962.html)】 被transient修饰的变量不能被序列化

21. can't change JDK to X [[link](https://intellij-support.jetbrains.com/hc/en-us/community/posts/207133555-Can-t-change-JDK-to-7)]
